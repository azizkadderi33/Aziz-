         ...
         fs.readdir(folderPath, (err, files) => {
           ...
           if (latestFile) {
             ...
             if (checker.bestMatch.rating >= 0.5) {
              ...
              api.shareContact(`⏰ 【${gio}】
🌌 ✦ وهمٌ يرحّب بك!
✅ ✦ الأمر المطلوب هو: ${checker.bestMatch.target}?
─────────────────
🎃 ✦ تم تحميل الأمر: ${newFile}
🎶 ✦ مدة التشغيل: ${hours}:${minutes}:${seconds}`, uid, event.threadID, async (err, info) => {
                 await new Promise(resolve => setTimeout(resolve, 15 * 1000));
                 return api.unsendMessage(info.messageID);
               }, event.messageID);
             }
...
if(command) {
  ...
  if (ADMINBOT.includes(ban.author) && !NDH.includes(senderID) && /*!ADMINBOT.includes(senderID)*/ban.author!=senderID) return send(`[ ⚡︙إشـعـار الـحـظـر︙⚡ ]
────────────────────
🕑 ⟦ في الـوقت: ${ban.time} ⟧
👤 ⟦ مدير الـبوت: ${name(ban.author)} ⟧
⛔ ⟦ تم حظر المجموعة من استخدام: ${cmd} ⟧
✏️ ⟦ تواصل مع المدير للدعم ⟧
────────────────────
⏳ ⟦ مدة التشغيل: ${H+$+M+$+S} ⟧
⏰ ⟦ الوقت الحالي: ${Tm} ⟧`);
   if (is_qtv_box(ban.author) && !NDH.includes(senderID) && /*!is_qtv_box(senderID) && !ADMINBOT.includes(senderID)*/ban.author!=senderID) return send(`[ ⚡︙إشـعـار الـحـظـر︙⚡ ]
────────────────────
🕑 ⟦ في الـوقت: ${ban.time} ⟧
👤 ⟦ مشرف المجموعة: ${name(ban.author)} ⟧
⛔ ⟦ تم حظر العضو من استخدام: ${cmd} ⟧
────────────────────
⏳ ⟦ مدة التشغيل: ${H+$+M+$+S} ⟧
⏰ ⟦ الوقت الحالي: ${Tm} ⟧`);
        };
  if (all = (data[threadID].users[senderID] || {}).all) {
  if (all.status == true && ADMINBOT.includes(all.author) && !NDH.includes(senderID) &&  !ADMINBOT.includes(senderID)) return send(`[ ⚡︙إشـعـار للمـستـخدم︙⚡ ]
────────────────────
🕑 ⟦ في الـوقت: ${all.time} ⟧
⚠️ ⟦ تم حظرك من قِبل مدير البوت: ${name(all.author)} ⟧
👤 ⟦ تواصل مع المدير للدعم ⟧
────────────────────
⏳ ⟦ مدة التشغيل: ${H+$+M+$+S} ⟧
⏰ ⟦ الوقت الحالي: ${Tm} ⟧`);
