...
module.exports.handleReaction = ({ api, event, handleReaction }) => {
  ...
  if (response == handleReaction.answer) api.sendMessage("-  ࢪاك صحيح🙄✊🏻💗!", event.threadID, () => {

          setTimeout(function(){ api.unsendMessage(handleReaction.messageID); }, 5000);
        });

  else api.sendMessage("-  ࢪاك غالط خو😹🤘🏻  !", event.threadID);
...
module.exports.run = async ({  api, event, args }) => {
  ...
  if (!fetch.data) return api.sendMessage("⚡️ بوت تناك🤘🏻💔", event.threadID, event.messageID);
  ...
  if (err) return api.sendMessage("- بوت تناك 🤘🏻💔", event.threadID, event.messageID);
  var retrieve = JSON.parse(body);
  var text = '';
  retrieve[0].forEach(item => (item[0]) ? text += item[0] : '');
  var fromLang = (retrieve[2] === retrieve[8][0][0]) ? retrieve[2] : retrieve[8][0][0]
  return api.sendMessage(`- ${text}\n\n   👍: يب       😢: غالط`, event.threadID, async (err, info) => {
    ...
    if (data.answerYet !== 1) {
      api.sendMessage(`- خلااص الوقت , وࢪاك صحيح خوو.🥺🤘🏻〘〙 : ${fetch.data.results[0].correct_answer}`, event.threadID, info.messageID);
