...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("طاغي ڪاش واحد🙍🏿‍♂️💔", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "افــضل اصدقاء دوملڪم ان شاء الله 😻‼️",
            mentions: [{
          tag: tag,
          id: mention
        }],
