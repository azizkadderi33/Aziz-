...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("حط ايدي لي حاب دخلو");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out("ملقيتش مولا ڪونت بلاك محذوف شوف وخداخࢪ")
			else {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`سـيد ${name ? name : "عضو"}  اه داخل اصلا فلغࢪوب.`);
		else {
			...
			try {
				...
				return out(` مقدࢪتش ندخا سيد ${name ? name : "عضو"} فلغࢪوب.`);
			}
			if (approvalMode === true && !admins.includes(botID)) return out(`صايي اني دخلتو لغࢪوب   ${name ? name : "عضو"}   اڪت !`);
			else return out(`صايي اني دخلتو   ${name ? name : "عضو"} لغࢪوب !`)
		}
	}
}